...
- 'throw': A genuine overhand pitching motion. Faces the throw direction, winds the elbow back past facing directly backward (rocking the shoulder back too), then flings both forward fast - resetting effectively to the faced pose - releasing the gripper mid-swing rather than after the arm stops. Not 'dropoff's careful staged descent. Parameters: 'target' (string, required) — the object currently held; must be the object the preceding 'pickup' grasped, this is verified and the step fails rather than guessing if it isn't; exactly one of 'destination' (string) — where to throw it toward — or 'direction' (string: 'forward'/'backward'/'left'/'right') — same meaning as for 'push'. Note: 'destination'/'direction' only decide which way the arm faces before swinging - the actual distance thrown is governed by 'wind_up_angle'/'fling_angle'/'speed', not by 'distance' or how far away the destination is. Optionally: 'distance' (float, meters) — with 'direction', how far out to aim the facing, defaults to 0.3; 'wind_up_angle' (float, radians) — how far the elbow swings back past facing directly backward, defaults to 3.927 (225 degrees: 180 to face backward, plus 45 further); 'fling_angle' (float, radians) — how far the elbow swings forward past the faced pose on the fling, on top of resetting there - defaults to 0.0 (resets effectively to the faced pose), raise it for an even harder throw; 'joint_2_rock_angle' (float, radians) — how far the shoulder rocks back/forward in step with the elbow, to exaggerate the motion, defaults to 0.2618 (15 degrees); 'release_delay' (float, seconds) — how long after starting the fling to release the gripper, defaults to roughly half the estimated fling duration (scaled to the angles/speed above) - only set this directly if that default timing looks wrong once observed; 'open_position' (float, 0.0-1.0) — how far the gripper opens on release, defaults to 0.0; 'speed' (float, 0.0-1.0) — the fling's speed specifically, defaults to 1.0 (full speed).
...
- Before choosing 'direction'+'distance' for 'push', check the object's position against Table Boundaries: if continuing that far in that direction would carry it past the table's edge, lower 'distance' to land just inside the boundary instead, unless the command explicitly asks for it to go off/over the edge (e.g. "push it off the table," "knock it onto the floor") - in that case honor the command as given rather than refusing or silently capping it. For 'throw', 'distance' only affects which way the arm faces before swinging, not how far the object actually lands - use 'wind_up_angle'/'fling_angle'/'speed' to reason about how far/hard to throw instead (bigger/faster for "throw it hard" or a farther destination, smaller/slower for "toss it gently").
